...
![漫画作成 - ストーリー選択画面](../wireframes/TopicChoice2.drawio.png)
...
| No | 項目 | 説明 |
|----|------|------|
| 1 | ヘッダー | サービス名ロゴ、サイドバー切り替えボタン、検索バー、次へボタン、ヘルプボタン、設定ボタン、アカウントボタン、ログアウトボタンを表示 |
| 2 | サイドメニュー | 各画面へ遷移するためのボタンを表示（切り替えボタンで表示・非表示を切り替え可能） |
| 3 | ストーリー一覧エリア | 登録済みストーリーをタイトル・あらすじ・ジャンル付きのカード形式で一覧表示し、1件選択できる |
| 4 | 素材フォルダ一覧エリア | イラスト素材フォルダをカード形式で一覧表示し、選択・追加・削除ができる。フォルダをクリックするとフォルダ内の素材一覧を表示する |
...
| フォルダカードをクリック（追加・削除ボタン以外の領域） | 該当フォルダを選択状態にし、漫画作成-編集画面（CreateManga）の素材一覧と同じUIで、フォルダ内のイラスト素材一覧を表示する |
| フォルダの追加ボタンをクリック | 新規の素材フォルダを作成する |
| フォルダの削除ボタンをクリック | 該当フォルダを削除する（削除前に確認ダイアログを表示） |
| 検索バーに入力・検索ボタンをクリック | 検索条件に一致するストーリーを表示する |
| ヘルプボタンをクリック | 本画面の操作説明を表示する |
...
- イラスト素材をフォルダに追加する際のアップロード専用画面は用意しない方針。OS標準のファイル選択ダイアログ（エクスプローラー等）を開いて選択する想定。
- フォルダクリック時に表示される素材一覧のUIは、漫画作成-編集画面（[CreateManga.md](CreateManga.md)）の素材一覧と共通化する想定。表示のみか、この画面からも素材の追加・削除が可能かは別途確定する。
