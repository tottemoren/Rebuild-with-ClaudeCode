...
| 7 | 見開き書き出しボタン | 表示中のページと次のページを1枚の横長画像（PNG）に合成し、端末にダウンロードするボタン |
...
| 見開き書き出しボタンをクリック | 表示中のページとその次のページを1枚の横長画像に合成してPNG形式で書き出し、端末にダウンロードする |
...
- 見開き書き出し時、次のページが存在しない（最終ページ単独の）場合の挙動は別途確定する（例: 単独ページのみで書き出す／ボタンを非活性にする等）。
...
- **見開き書き出し機能について**：他ユーザーへの作品公開機能（一覧・詳細閲覧画面等）は将来的に追加予定だが、初回リリースではスコープ外とする。代わりに、完成したページを見開き（隣接する2ページ）単位でPNG画像として端末にダウンロードできる本機能を用意し、ユーザーが任意の手段（SNS等）で共有できるようにする。画像はサーバー・S3には保存しない（クライアント側のみで完結する機能）。
  - ページの組み合わせ方（1-2ページ目、3-4ページ目…の固定ペアか、表示中ページ+次ページの動的な組み合わせか）は別途確定する。
  - 類似機能（固定枠キャンバスをPNG書き出しする「画像として保存」ボタン）は`RebuildJava`ではなく`RebuildReact`側に実装済み（[CreateMangaPage.tsx](../../../RebuildReact/src/pages/CreateMangaPage/CreateMangaPage.tsx)）。ただし現行実装は単一ページ・単一キャンバスのみで、複数ページ・見開き合成には未対応。
