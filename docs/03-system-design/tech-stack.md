...
| CI/CD | GitHub Actions（有力候補・ツール最終選定は未確定） | mainブランチへのマージを起点に「ビルド → テスト → Dockerイメージのビルド/ECRへのpush → ECSサービス更新」と「フロントエンドのビルド成果物のS3デプロイ → CloudFrontキャッシュ無効化」を自動化する想定 |
...
| CI/CDパイプライン | **最終的に導入する方針で決定済み**。ツールは未選定（GitHub連携済みのため GitHub Actions が有力候補）。パイプライン構成の詳細は本番環境構築とあわせて設計する |
