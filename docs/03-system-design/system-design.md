...
## 5. 完成作品の公開範囲（初回リリースのスコープ）

### 5.1 背景

01-requirementsの著作権方針や、04-databaseでの素材共有設計（`folder_shares`）の議論からもわかる通り、将来的には他ユーザーが作品を閲覧できる公開機能（一覧・詳細閲覧画面等）を追加する構想がある。ただし、初回リリースをできるだけ早く出すことを優先し、公開機能自体は初回リリースのスコープ外とする。

### 5.2 方針：見開き単位のPNG書き出し（クライアント完結）

公開機能の代わりに、[SCR-007 漫画作成-編集画面](../02-screen-design/specifications/CreateManga.md)に「見開き書き出し」機能を用意する。

| 項目 | 方針 |
|---|---|
| 処理内容 | 表示中のページとその次のページを1枚の横長画像に合成し、PNGとして書き出す |
| 保存先 | 端末へのダウンロードのみ。サーバー・S3への保存は行わない |
| 共有方法 | ダウンロードした画像をユーザー自身がSNS等で任意に共有する（アプリ外での共有） |
| 実装方針 | クライアント（`RebuildReact`）側のCanvas処理のみで完結させる。バックエンドの新規実装は不要 |

既存実装（[CreateMangaPage.tsx](../../RebuildReact/src/pages/CreateMangaPage/CreateMangaPage.tsx)）に単一ページ・単一キャンバスをPNG書き出しする「画像として保存」ボタンが既にあり、これを複数ページ・見開き合成に対応させる拡張として位置づける。

### 5.3 将来の拡張

公開機能（一覧・詳細閲覧画面、閲覧数、公開範囲設定等）を追加する際は、書き出した画像をS3に保存する方式に切り替える必要がある。その際は04-databaseに公開作品を管理するテーブル（例: 公開URL、公開日時、閲覧可否等）を追加で設計する。
...
デプロイの自動化（CI/CD）は**最終的に導入する方針で決定済み**。GitHubへの連携が既にあるため GitHub Actions を有力候補とし、mainブランチへのマージを起点に「ビルド → テスト → Dockerイメージのビルド/ECRへのpush → ECSサービス更新」および「フロントエンドのビルド成果物のS3デプロイ → CloudFrontキャッシュ無効化」を自動化する想定。具体的なパイプライン構成・ツールの最終選定は本番環境構築とあわせて行う（[tech-stack.md](tech-stack.md)参照）。
...
- プロフィール画像のS3移行要否の判断
- CI/CDパイプラインの詳細設計（ツールの最終選定・パイプライン構成）※導入方針は決定済み（6.1参照）
- IaC（Terraform等）、監視・ログ収集（CloudWatch等）の検討
- 見開き書き出し機能におけるページの組み合わせ方（固定ペアか、表示中ページ+次ページの動的な組み合わせか）の確定
- 将来、公開機能を追加する際の画像保存方式（S3移行）・公開範囲管理テーブルの設計
